...
select ord_num, ord_amount, agent_code from orders;


/* Problem 1 :
Write a SQL query that retrieves the names of the 5 agents with the least total order amounts.
...
select agent_name, total_ord_amount,
rank() over(order by total_ord_amount asc) as ranking
from
(select a.agent_code, a.agent_name, sum(ord_amount) as total_ord_amount
from agents a
left join orders o
on a.agent_code = o.agent_code
group by a.agent_code, a.agent_name) t1
limit 5;


/* Problem 2 :
However, it has been noted that data provided by the agent named "Mukesh" is not reliable, so any data associated with this agent
should be excluded from the analysis.
Write a SQL query that retrieves the name of the customer with the
minimum outstanding amount from each country. The output should include the
following columns:
cust_country: The country of the customer.
cust_name: The name of the customer with the minimum outstanding amount in
that country. */
select cust_country, cust_name from
(
select *,
rank() over(partition by cust_country order by outstanding_amt asc) ranking
from
(
select cust_country, cust_name, outstanding_amt from customer
where agent_code <> (select agent_code from agents where agent_name='Mukesh')
) t1
) t2
where ranking =1;
...
/* Problem 3 :
Write a SQL query that calculates the running total of the payment_amt for each cust_city, ordered by the grade in ascending order. 
The running total should include the payment_amt of the current row plus the payment_amt from the next two rows.
The output should include the following columns:
...
sum(payment_amt) over(partition by cust_city order by grade asc rows between current row and 2 following) running_tot
from customer;


/* Problem 4 :
- If the average advance amount collected by an agent is between 750 and 1000 (inclusive): The new commission will be 0.9 times the 
current commission.
- If the average advance amount collected by an agent is more than 1000: The new commission will be 1.2 times the current commission.
The output should include the following columns:
agent_code: The code of the agent.
updated_commission: The newly calculated commission based on the rules
above. */
select agent_code, 
case when advance_amount between 750 and 1000 then commission*0.9
	 when advance_amount>1000 then commission *1.2
else commission
end updated_commission
from 
(select a.agent_code, o.advance_amount, a.commission from agents a
left join orders o
on a.agent_code=o.agent_code) t1; 
